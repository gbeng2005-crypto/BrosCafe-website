...
const PANELS = [
  { img: UPLOADS.coffeePoster, label: "COFFEE", caption: "Pulled slow, served warm.", product: "espresso" },
  { img: UPLOADS.sweetsPoster, label: "SWEET", caption: "Because coffee deserves company.", product: "croissant" },
  { img: IMAGES.about, label: "PEOPLE", caption: "Regulars became friends.", product: null },
  { img: "https://images.unsplash.com/photo-1556821840-3a63f95609a7?w=900&q=80", label: "MERCH", caption: "Take a little Bros home.", product: "bros-hoodie" },
];
  ...
  const panels = (
    <>
      {PANELS.map((p, i) => (
        <button
          key={p.label}
          data-testid={`discover-panel-${p.label.toLowerCase()}`}
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 p-7 md:p-9">
            ...
            <p className="font-serif-display mt-1 text-4xl font-medium text-[#F5F0E6] md:text-6xl">{p.label}</p>
            <p className="mt-2 text-sm italic text-[#F5F0E6]/85">{p.caption}</p>
          </div>
        </button>
      ))}
      <div className="flex h-[62vh] w-[80vw] shrink-0 flex-col items-start justify-center rounded-3xl bg-[#66734A] p-8 md:h-[70vh] md:w-[52vw] md:p-14">
        <p className="font-serif-display text-4xl font-medium leading-tight text-[#F5F0E6] md:text-6xl">
          Come hang
          <br />
          out<span className="italic">.</span>
        </p>
        <a
    ...
    return (
      <section id="discover" data-testid="discover-section" className="bg-white py-24">
        <div className="mx-auto max-w-[1440px] px-6 md:px-12">
          <SectionLabel number="05" title="DISCOVER BROS" />
        </div>
      ...
      <div className="sticky top-0 flex h-screen flex-col justify-center overflow-hidden">
        <div className="mx-auto w-full max-w-[1440px] px-6 pb-8 md:px-12">
          ...
          <h2 className="font-serif-display text-4xl font-medium tracking-tight text-[#66734A] md:text-6xl">
            SCROLL SIDEWAYS<span className="italic">.</span>
          </h2>
        </div>
